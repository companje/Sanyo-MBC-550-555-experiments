//void v_wrap(int p) {
//  mem(p+0).set(wrap_word(mem(p+0).get()));
//  mem(p+1).set(wrap_word(mem(p+1).get()));
//}
...
//  ax.set(ax.get()+360);  //make positive
//  ax.set(ax.get()%360);
//  ax.set(ax.get()/15);
//  ax.set(v[ax.get()]);
//}
...
//  pop bx
//  ret
//.lut[] = db { 0,25,50,70,86,96,100,96,86,70,50,25,0,-25,-50,-70,-86,-96,-100,-96,-86,-70,-50,-25,0 };



}

void angle_lookup() { // bx=lookup table, ax=angle (>=0) 0..360, result in ah
  //angle_wrap();  //makes angle between 0..360
  //dx.push();
  //bx.push();
