...
int COLS=80;
int ROWS=50;
int BAR=4*COLS;  // bytes per row: 4 lines per bar (80=360, 72=288)
...
int upperFont[] = {
  ...
  0x00, 0x00, 0x01, 0x03, 0x00, 0x00, 0x80, 0xc0,
  0x00, 0x00, 0x03, 0x07, 0x00, 0x00, 0xc0, 0xe0,
  0x00, 0x00, 0x07, 0x0f, 0x00, 0x00, 0xe0, 0xf0,
  0x00, 0x03, 0x0f, 0x1f, 0x00, 0xc0, 0xf0, 0xf8,
  0x00, 0x07, 0x1f, 0x3f, 0x00, 0xe0, 0xf8, 0xfc,
  0x01, 0x0f, 0x3f, 0x3f, 0x80, 0xf0, 0xfc, 0xfc,
  0x03, 0x1f, 0x3f, 0x3f, 0xc0, 0xf8, 0xfc, 0xfc,
  0x07, 0x1f, 0x3f, 0x7f, 0xe0, 0xf8, 0xfc, 0xfe,
  0x07, 0x1f, 0x7f, 0x7f, 0xe0, 0xf8, 0xfe, 0xfe,
  0x07, 0x3f, 0x7f, 0x7f, 0xe0, 0xfc, 0xfe, 0xfe,
  0x0f, 0x3f, 0x7f, 0x7f, 0xf0, 0xfc, 0xfe, 0xfe,
  0x0f, 0x3f, 0x7f, 0xff, 0xf0, 0xfc, 0xfe, 0xff,
  0x1f, 0x7f, 0xff, 0xff, 0xf8, 0xfe, 0xff, 0xff};
  
int lowerFont[] = new int[upperFont.length];
...
void draw() {
  ...
  //si=R+top;
  //do {
  //  for (int i=0; i<upperFont.length; i++) {
  //    mem[si+i] = upperFont[i];
  //  }
  //  si+=2*BAR;
  //} while (++cx<16);
  ...
  //  //    for (int b=0; b<4; b++) {
  //  //      mem[row*2*BAR + BAR + G+top + col*cell*b] = 255;
  //  //    }
  //  //  }
  //  //}
  //  for (int b=0; b<2*4*16; b++) {
  //     //mem[row*2*BAR + G+top+BAR+b]=255;
  //  }
  //}
  ////for (int col=0; col<16; col++) {
  ////  //bx = top + row*col*4;
  ////  //do {
  ////    for (int i=0; i<3; i++) {
  ////      mem[G+bx+BAR+i] = mem[R+bx+2-i];
  ////    }
  ////  //mem[G+bx+BAR+1] = mem[R+bx+1];
  ////  //mem[G+bx+BAR+2] = mem[R+bx+0];
  ////  //mem[G+bx+BAR+3] = 0;
  ////  bx+=8;
  ////}
  ////bx+=mouseX;
