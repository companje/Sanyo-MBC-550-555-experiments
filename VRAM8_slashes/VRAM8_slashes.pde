...
void setup() {
  ...
  slash = loadBytes("slash.spr");
  
  setCellRed(10,10,255);
  drawSprite(10,10,slash);
  
  //setMemoryFromImage()
}

void drawSprite(int row_offset, int col_offset, byte b[]) {
  int w = b[0]; //bijv 4 for 4 cells
  int h = b[1];
  for (int y=0; y<h; y++) {
    for (int x=0; x<w; x++) {
      for (int q=0; q<4; q++) {
  ...
  //si=2;
  //di=0;
  //drawChannel(R,si,di,b);
  //drawChannel(G,si,di,b);
  //drawChannel(B,si,di,b);
}

//void drawChannel(int ch, int si, int di, byte b[]) {
//  stack.push(di);
////  mov es,ax
//  for (int row=0; row<
...
void setCell(int row, int col, int brightness, int channel) {
  //b = constrain(b/32, 0, 7); //deze regel kan uit bij 255 resolutie
  for (int i=0; i<4; i++) {
    mem[channel+row*COLS*4 + col*4 + i] = intensity[brightness][i];
  }
}
...
//      float x=col; //float(col)/COLS;
//      float y=row; //float(row)/ROWS;
//      float t = frameCount*.1;
//      //float tixy = sin(x)*cos(y);
//      //cos((x-COLS/2)/3*sin(t/4))*sin((y-ROWS/2)/3*sin(t/4)) ; //(row&col)+t*i); ////sin(xx/8*yy/5 + t)*
//      //cos(t+(xx/4*yy/5));
...
//      //float tixy = sin(xr); // * cos(yr);
  
//      float tixy = sin(yr*xr+pow(t,sin(t*t)));


//      int o = int(constrain(map(tixy, -1, 1, 0, 255), 0, 255));


//      setCellGreen(row, col, o);


//      tixy = cos(xr); // * cos(yr);
//      o = int(constrain(map(tixy, -1, 1, 0, 255), 0, 255));
//      setCellRed(row, col, o);

//      tixy = sin(xr)* cos(yr);
//      o = int(constrain(map(tixy, -1, 1, 0, 255), 0, 255));
//      setCellBlue(row, col, o);
