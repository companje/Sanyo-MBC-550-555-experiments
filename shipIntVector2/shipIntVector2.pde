...
void warp(Vector p) {
  if (p.y-player.pos.y-world_h+height > 0) p.y-=world_h;
  if (p.y-player.pos.y+height < 0) p.y+=world_h;
  if (p.x-player.pos.x-world_w+width > 0) p.x-=world_w;
  if (p.x-player.pos.x+width < 0) p.x+=world_w;
}
