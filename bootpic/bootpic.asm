...
draw_char:    ; bh=row, bl=col, al=char
  push ax
  ...
  mov cx,8
  mul cx
  mov si, ax ;ax ;ax          ; Offset voor karakter 0x01 in de font-tabel (0x01 * 8 bytes = 0x08)

  ; mov di, 0  should be set in advance
  mov cx, 4            ; Aantal bytes voor de bovenste helft
  rep movsb            ; Kopieer de eerste 4 bytes van DS:SI naar ES:DI
  
  add di, 4*72-4         ; Zet DI naar de positie voor de onderste helft (4 rijen verder)
