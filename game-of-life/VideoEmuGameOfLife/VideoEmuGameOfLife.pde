...
void setDot(int x, int y) {
  for (int j=0; j<4; j++)
    mem[GREEN + y*COLS*4+x*4 + j] = 255;
}

void init() {
  for (int i=0; i<WH; i++) {
    int v = random(1)<.5 ? 255 : 0;
    for (int j=0; j<4; j++)
      mem[GREEN + i*4+j] = v;
  }
...
void nextgen() {
  ...
  for (int i=0; i<WH; i++) {
    ...
    mem[GREEN + i*4] ^= b; //xor: toggle bit 5 or 6 based on num neighbours
  }
  ...
  for (int i=0; i<WH; i++) {
    ...
    for (int j=0; j<4; j++)
      mem[GREEN + i*4+j] = nextState ? 255 : 0;
  }
}
...
int countNeighboursUpTo3(int p) {
  ...
  for (int i=0; i<nb.length; i++)
    if (mem[GREEN + p*4 + nb[i]*4]>=128) n++; // && ++n>3  count to max 3 for bit7==1
  return n; //0,1 or 2
}
